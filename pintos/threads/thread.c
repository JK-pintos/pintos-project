...
void thread_yield(void) {
    ...
    old_level = intr_disable();
    if (curr != idle_thread) list_insert_ordered(&ready_list, &curr->elem, thread_priority_order, NULL);
...
thread_set_priority(int new_priority) {
    ...
    if (!list_empty(&ready_list)) {
        struct thread *highest_ready = list_entry(
            list_max(&ready_list, thread_priority_order, NULL),
            struct thread, elem);
...
/* Compare two threads by priority */
bool thread_priority_order(const struct list_elem* a, const struct list_elem* b, void* aux){
...
/* Compare two donated threads by priority */
bool thread_donation_priority_order(const struct list_elem* a, const struct list_elem* b, void* aux){
    const struct thread* thread1 = list_entry(a, struct thread, donation_elem);
    const struct thread* thread2 = list_entry(b, struct thread, donation_elem);
    return thread1->priority > thread2->priority;
}
